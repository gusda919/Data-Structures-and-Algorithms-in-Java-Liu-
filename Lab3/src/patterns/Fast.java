...
public class Fast {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// "Replace <input.txt> with your input file of preference, and possibly the
		// path.\n" +
		...
		f = new File("/home/gusda919/TDDE22/laborationer/Lab3/src/data/input12800" + ".txt");
		try {
			input = new Scanner(f);
		...
		for (int i = 0; i < count - 1; i++) {
			...
			HashMap<Double, ArrayList<Point>> slopeValues = new HashMap<Double, ArrayList<Point>>();

			for (int j = i + 1; j < count; ++j) {

				double slope = points.get(i).slopeTo(points.get(j));

				if (slopeValues.containsKey(slope)) {
					ArrayList<Point> slopeList = slopeValues.get(slope);
					slopeList.add(points.get(j));

				}

				else {
					ArrayList<Point> slopeList = new ArrayList<Point>();
					slopeList.add(points.get(j));
					slopeValues.put(slope, slopeList);
				}
			}

			for (double key : slopeValues.keySet()) {
				ArrayList<Point> slopeList = slopeValues.get(key);
				if (slopeList.size() >= 3) {
